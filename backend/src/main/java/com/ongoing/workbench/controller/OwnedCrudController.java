...
public abstract class OwnedCrudController<T extends OwnedEntity, ID> {
    ...
    private static final Logger log = LoggerFactory.getLogger(OwnedCrudController.class);
    ...
    /**
     * POST 请求体缺 id 时补一个 UUID。
     * 主键是手工赋值型（无 @GeneratedValue），缺 id 直接 save 会被 JPA 当成新记录 persist，
     * 抛 "Identifier must be manually assigned before calling persist()" → 500。
     * 前端正常路径都会自生成 id（store.create 的 uid()），这里是给第三方调用兜底。
     */
    private void ensureId(T entity) {
        Field f = findIdField(entity);
        if (f == null || f.getType() != String.class) return;
        try {
            f.setAccessible(true);
            if (f.get(entity) == null) {
                f.set(entity, UUID.randomUUID().toString());
                log.warn("创建 {} 的请求缺少 id，服务端已补 UUID 兜底", entity.getClass().getSimpleName());
            }
    ...
    /** 沿继承链查找 @Id / @EmbeddedId 字段（主键在各实体自己类里声明，基类看不到） */
    private Field findIdField(T entity) {
